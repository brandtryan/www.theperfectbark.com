import { linesArray } from './modules/linesArray.js';

let ln = [];
let line0wordAnimations = [];
const DURATION = 1000;
let section, paragraph, line;
...
for (let line of linesArray) {
  ...
  // Give each new LINE span a unique id
  line.id = `L${i}`;
  // Give each new LINE span the line-container class
  line.className = 'line';
  // Append the LINE spans to the main element
  paragraph.appendChild(line);

  // Create variables for each line
  line[i] = document.getElementById(`ln${i}`);
...
// const wordArray0 =
//   `My teachers look at me funny when I make weird noises,`.split(/(\s+)/);
// const wordArray1 =
//   `but my friends, they usually just laugh. It's funny to them,`.split(/(\s+)/);
// const wordArray2 =
//   `and they are pretty cool about it – they don't make fun of`.split(/(\s+)/);
// const wordArray3 =
//   `me really, but only get a kick out of it – like when we are in`.split(
//     /(\s+)/
//   );
// const wordArray4 =
//   `the middle of a test or something and then out of nowhere`.split(/(\s+)/);
// const wordArray5 =
//   `I let out a bark – I mean a loud bark – where everyone turns`.split(/(\s+)/);
// const wordArray6 =
//   `around to me and smiles, and then someone starts laughing`.split(/(\s+)/);
// const wordArray7 =
//   `and then everyone else gets going. To tell you the truth it`.split(/(\s+)/);
// const wordArray8 =
//   `doesn't bother me at all and I laugh right along with them`.split(/(\s+)/);
// const wordArray9 = `most of the time.`.split(/(\s+)/);

// Create function to create word arrays for each line

// Use array.length/index for appending number to end
...
// pauseArray(line0wordAnimations);
// playArray(line0wordAnimations);

// console.log(
//   'ln0w10 duration = ' + line0wordAnimations[0].effect.getTiming().duration
// );
// console.log(
//   'ln0w18 duration = ' + line0wordAnimations[1].effect.getTiming().duration
// );
// console.log('ln0w10 startTime = ' + line0wordAnimations[0].startTime);
// console.log('ln0w18 startTime = ' + line0wordAnimations[1].startTime);
// console.log(
//   'ln0w10 WHOLE duration = ' + overall_duration(line0wordAnimations[0])
// );
// console.log(
//   'ln0w18 WHOLE duration = ' + overall_duration(line0wordAnimations[1])
// );
// console.log('Line Reading Time = ' + lineDuration0);

// MISC FUNCTIONS

// Play array of animations
function pauseArray(elements) {
  elements.map((element) => element.pause());
}

// Play array of animations
function playArray(elements) {
  elements.map((element) => element.play());
}
